...
public class CloseLRARunnable implements Runnable {
    ...
    public void run() {
        try {
            logger.info("Starting to close LRA thread --> Wait for 1 minute before sending the request, so the ID " +
                    "exists.");
            // wait for one minute before trying to close the active LRA since it has just been created
            Thread.sleep(60000);
